...
const mintAssets = async (
  ...
  try {
    ...
    const txIdHex = walletData.utxos[0][0].txId;
    j.log({ txIdHex });
    const utxoIdx = walletData.utxos[0][0].utxoIdx;
    j.log({ utxoIdx });
    ...
    tx.addOutput(
      new hlib.TxOutput(
        ...
        new hlib.Value(
          minUTXOVal.lovelace,
          new hlib.Assets([[nftCompiledProgram.mintingPolicyHash, nft]])
        )
      )
    );
    tx.addMetadata(721, {
...
export const mint = async (tokenName, tokenDescription, tokenImageUrl) => {
  ...
  const walletData = await walletEssentials(
    wallet,
    Cip30Wallet,
    WalletHelper,
    Value,
    txPrerequisites.minAda,
    j
  );
  j.log({ walletData });
  j.test(
    "coxylib testPack",
    "walletEssentials walletEnabled",
...
export async function init(j) {
  ...
  try {
    ...
    if (typeof cwindow != "undefined") {
      ...
      alert(
        "Sorry you need to connect to browsers wallets: Nami/Eternl/Lace in preprod network"
      );
...
export async function walletEssentials(
  selectedWallet,
  Cip30Wallet,
  WalletHelper,
  Value,
...
export const showWalletData = async (walletData, utxoAmount, AssetClass, j) => {
  ...
  try {
    ...
    const collateralAda = String(
      (await walletData.walletHelper.pickCollateral()).value.lovelace /
        BigInt(1000000)
    );
    j.log({ collateralAda });
    const shortAddress =
    ...
    const assets = Object.values(
      Object.values(utxos[0])
    )[0].value.assets.dump();
    j.log({ assets });
    const assetsArray = Object.keys(assets).map((key, value) => [
      key,
      assets[key],
    ]);
    j.log({ assetsArray });
    const assetsAsStringArray = JSON.parse(JSON.stringify(assetsArray));
    j.log({ assetsAsStringArray });
...
export async function getAssets(asset, j) {
  var res = await fetch(
    "https://coxygen.co/pacman-cardano/api.php?asset=" + asset,
    {
      method: "GET",
      headers: {
...
export const txFunc = async (walletData, hlib, tx, j, networkParamsUrl) => {
  ...
  const txHash = await walletData.walletAPI.submitTx(tx);
  j.log({ txHash: txHash.toString() });
...
export const addSigner = async (
  ...
  await tx.finalize(networkParams, partyAddress, partyAutxos);
  j.log({ tx });
  const partySignature = await partyAddress.signTx(tx);
  j.log({ partySignature });
  tx.addSignatures(partySignature);
  j.log({ tx });
  if (owner != "") {
    const ownerSignature = await owner.signTx(tx);
    j.log({ ownerSignature });
...
export const mintTokens = async (tx, mph, token, redeemer, j) => {
  await tx.mintTokens(mph, token, redeemer);
  j.log({ tx });
  return tx;
};

export const txRefInput = async (tx, utxo, j) => {
  await tx.addRefInput(utxo);
  j.log({ tx });
  return tx;
};
